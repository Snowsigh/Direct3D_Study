...
HRESULT KVertex::CreateVertexBuffer()
{
    ...
    hr = m_pd3dDevice->CreateBuffer(&pDec, &pInitData, &m_pVertexBuffer);
...
HRESULT KVertex::LoadShaderAndInputLayout()
{
    ...
    hr = m_pd3dDevice->CreateInputLayout(pInputLayout, numLayout, m_pVStemp->GetBufferPointer(), m_pVStemp->GetBufferSize(), &m_pVertexLayout);
    if (FAILED(hr)) return hr;
...
HRESULT KVertex::LoadShader()
{
    ...
    hr = m_pd3dDevice->CreatePixelShader(m_pPStemp->GetBufferPointer(), m_pPStemp->GetBufferSize(), NULL, &m_pPS);
    if (FAILED(hr)) return hr;
    m_pPStemp->Release();

    hr = m_pd3dDevice->CreateVertexShader(m_pVStemp->GetBufferPointer(), m_pVStemp->GetBufferSize(), NULL, &m_pVS);
    if (FAILED(hr)) return hr;
...
bool	KVertex::Render()
{
    ...
    m_pImmediateContext->IASetVertexBuffers(0, 1, &m_pVertexBuffer, &pStrides, &pOffsets);
